        ...
        <div id="board">
                ...
                <c:forEach items="${boardList}" var="item" varStatus="index">
                    <tr>
                        ...
                        <td style="text-align: left; padding-left: ${item.depth*20}px">
                            ...
                            <a href="${pageContext.request.contextPath}/board/view?currentPage=${pagingData.currentPage}&id=${item.id}">${item.title}</a>
                        </td>
                        <td>${item.user_name}</td>
                        <td>${item.hit}</td>
                        <td>${item.reg_date}</td>
                        <c:if test="${sessionScope.get('authUser').id == item.user_id}">
                            <td><a href="${pageContext.request.contextPath}/board/delete/${item.id}"
                                   class="del">삭제</a></td>
                        </c:if>
                    </tr>
                </c:forEach>
            ...
            <div class="pager">
                <ul>
                    ...
                    <c:forEach begin="${pagingData.prevPage+1}" end="${pagingData.prevPage+pagingData.perPage}"
                               var="index">
                        <c:choose>
                            ...
                            <c:when test="${index != pagingData.currentPage && index <= pagingData.endPage}">
                                <li>
                                    <a href="${pageContext.request.contextPath}/board?page=${index}${not empty param.kwd?'&kwd='.concat(param.kwd):""}">${index}</a>
                                </li>
                            </c:when>
                            <c:otherwise>
                                <li>${index}</li>
